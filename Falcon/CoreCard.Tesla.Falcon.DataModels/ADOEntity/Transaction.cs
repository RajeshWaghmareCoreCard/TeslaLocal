...
namespace CoreCard.Tesla.Falcon.DataModels.Entity
{
    ...
    public class Transaction : BaseEntity
    {
        ...
        public static implicit operator Transaction(DataRow dr)
        {
            Transaction o = new Transaction();
            if (dr.IsNull("tranid") == false)
            {
                o.tranid = dr.Field<Guid>("tranid");
            }
            if (dr.IsNull("accountid") == false)
            {
                o.accountid = dr.Field<Guid>("accountid");
            }
            if (dr.IsNull("trantype") == false)
            {
                o.trantype = dr.Field<string>("trantype");
            }
            if (dr.IsNull("trancode") == false)
            {
                o.trancode = Convert.ToInt32( dr.Field<Int64>("trancode"));
            }
            if (dr.IsNull("trantime") == false)
            {
                o.trantime = dr.Field<DateTime>("trantime");
            }
            if (dr.IsNull("amount") == false)
            {
                o.amount = dr.Field<decimal>("amount");
            }
            if (dr.IsNull("cardnumber") == false)
            {
                o.cardnumber = dr.Field<string>("cardnumber");
            }
